      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->maxarea = (double) strtod(workstring, (char **) NULL);
            if (b->maxarea <= 0.0) {
...
void poolinit(struct memorypool *pool, int bytecount, int itemcount, enum wordtype wtype, int alignment)
{
  ...
  wordsize = (pool->itemwordtype == POINTER) ? sizeof(VOID *) : sizeof(double);
...
VOID *poolalloc(struct memorypool *pool)
{
  ...
  if (pool->deaditemstack != (VOID *) NULL) {
    ...
    if (pool->itemwordtype == POINTER) {
      ...
      pool->nextitem = (VOID *) ((double *) pool->nextitem + pool->itemwords);
    }
...
VOID *traverse(struct memorypool *pool)
{
  ...
  if (pool->itemwordtype == POINTER) {
    ...
    pool->pathitem = (VOID *) ((double *) pool->pathitem + pool->itemwords);
  }
  pool->pathitemsleft--;
  return newitem;
}
...
double orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   ...
                   double permanent)
{
  INEXACT double adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  double det, errbound;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, double *xi, double *eta, double *minedge)
{
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                   double angle, vertex enqapex, vertex enqorg, vertex enqdest)
{
...
int checkseg4encroach(struct mesh *m, struct behavior *b,
                      struct osub *testsubseg, double iradius)
{
  struct otri neighbortri;
  struct osub testsym;
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, double **hlist, int *holes,
               double **rlist, int *regions)
{
  double *holelist;
  double *regionlist;
  char inputline[INPUTLINESIZE];
  ...
  *holes = (int) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (double *) trimalloc(2 * *holes * sizeof(double));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    *regions = (int) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (double *) trimalloc(4 * *regions * sizeof(double));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
...
void triangulate(char *triswitches, spdlib::nn::triangulateio *in, spdlib::nn::triangulateio *out, spdlib::nn::triangulateio *vorout)
{
  ...
  if (vorout != (spdlib::nn::triangulateio *) NULL) {
